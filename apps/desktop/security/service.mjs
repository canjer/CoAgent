...
export class SecurityService{
 ...
 try{
  ...
  const loaded=await this.config(workspace);if(loaded.warning)throw new Error('配置从备份恢复，请核对并重新保存后执行');const config=loaded.config;if(!config.tools.includes(args.tool))throw new Error('工具未在项目中启用');
  ...
  const plan={version:1,tool:toolRegistry[args.tool],urls:args.urls,config,configHash:hash(JSON.stringify(config)),workspace:resolve(workspace),confirmedAt:new Date().toISOString()};await atomicWrite(join(dir,'plan.json'),JSON.stringify(plan,null,2));
 ...
 try{await event('started',{tool:plan.tool.id,configHash:plan.configHash});
  for(const url of plan.urls){signal.throwIfAborted();await headProbe({url,config:plan.config,signal,
   ...
   record:async value=>{const name=String(state.evidenceCount+1).padStart(4,'0')+'.json';const data=JSON.stringify({time:new Date().toISOString(),tool:plan.tool,...value},null,2);await atomicWrite(join(dir,'evidence',name),data);state.evidenceCount++;await event('evidence',{file:'evidence/'+name,sha256:hash(data)});await persist();}
