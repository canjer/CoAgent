...
export async function headProbe({url,config,signal,reserve,record,resolve=lookup}){
 ...
 for(let hop=0;hop<=5;hop++){
  ...
  const selected=addresses[0];const response=await new Promise((accept,reject)=>{
   const req=(u.protocol==='https:'?https:http).request(u,{method:'HEAD',agent:false,signal,lookup:(_host,options,cb)=>options.all?cb(null,[selected]):cb(null,selected.address,selected.family),headers:{'user-agent':'CoAgent-Security-S1/1.0','accept':'*/*'},maxHeaderSize:16384},res=>{
    const headers={};for(const key of ['content-type','content-length','server','strict-transport-security','x-content-type-options','x-frame-options'])if(typeof res.headers[key]==='string')headers[key]=res.headers[key].slice(0,1024);
    const value={url:u.href,status:res.statusCode,headers,location:res.headers.location};res.destroy();accept(value);
   });req.on('error',reject);req.setTimeout(config.budget.timeoutMs,()=>req.destroy(new Error('请求超时')));req.end();
  });
  const {location,...evidence}=response;await record({...evidence,address:selected.address});
