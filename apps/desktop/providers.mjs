...
export function validateProvider(v){
 ...
 const apiStyle=v.apiStyle??'openai';if(!['openai','deepseek'].includes(apiStyle))throw new Error('仅支持 OpenAI 或 DeepSeek API');
...
export class ProviderRegistry{
 ...
 async save(value){const id=value.id||'custom-'+randomUUID().replaceAll('-','');if(value.id&&!this.models.some(p=>p.id===id))throw new Error('待编辑服务已不存在，请关闭编辑器后重新添加');const profile=validateProvider({...value,id});const next=this.models.filter(p=>p.id!==id).concat(profile);const saved=await this.store.write({version:1,models:next});this.models=saved.models;return profile;}
