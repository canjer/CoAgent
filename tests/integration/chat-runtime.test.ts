...
test('real Codex + Chat bridge provider: read/write/test, history and restart', { timeout: 60000 }, async t => {
  ...
  const upstream = await testServer((body, _req, res) => {
    assert.equal(_req.url,'/v1/chat/completions');
    const tools = ((body.tools ?? []) as Obj[]).map(t=>t.function as Obj);
    const messages=body.messages as Obj[];
    const outputs=messages.filter(m=>m.role==='tool');
    let item:Obj;
    if(step===0)item=toolCall(tools,'cat input.txt',step++);
    else if(step===1){assert.match(JSON.stringify(outputs),/hello/);item=toolCall(tools,"printf 'coAgent test\\n' > output.txt; test -z \"$COAGENT_GATEWAY_TOKEN\" && printf 'TOOLS_OK\\n'",step++);}
    else{assert.match(JSON.stringify(outputs),/TOOLS_OK/);item=assistant('Chat fixture completed.');step++;}
    res.writeHead(200,{'content-type':'text/event-stream'});
    const chunk=(delta:Obj,finish_reason:string|null)=>`data: ${JSON.stringify({choices:[{index:0,delta,finish_reason}]})}\n\n`;
    if(item.type==='function_call'){
      const args=String(item.arguments),mid=Math.floor(args.length/2);
      res.end(chunk({tool_calls:[{index:0,id:item.call_id,function:{name:item.name,arguments:args.slice(0,mid)}}]},null)+chunk({tool_calls:[{index:0,function:{arguments:args.slice(mid)}}]},'tool_calls')+'data: [DONE]\n\n');
    }else res.end(chunk({content:'Chat fixture completed.'},'stop')+'data: [DONE]\n\n');
  }); cleanup.push(() => upstream.close());
  const gateway = await startGateway({ token, provider: { protocol: 'chat-completions', baseUrl: upstream.baseUrl, alias: 'coagent-test', upstreamModel: 'fixture-model' } }); cleanup.push(() => gateway.close());
  ...
  console.log('CHAT_REAL_RUNTIME_FIXTURE: read/write/test=PASS history/restart=PASS credential_environment=PASS');
  
});


test('Chat bridge real Codex cancellation reaches an interrupted terminal state', { timeout: 30000 }, async t => {
  const cleanup: (() => Promise<void>)[] = [];
  t.after(async () => { for (const close of cleanup.reverse()) await close(); });
  let observed!: () => void;
  const requestObserved = new Promise<void>(resolve => { observed = resolve; });
  const upstream = await testServer((_body, _req, res) => {
    if (_req.url !== '/v1/chat/completions') { res.writeHead(404); res.end(); return; }
    res.writeHead(200, { 'content-type': 'text/event-stream' }); res.write(': waiting\n\n'); observed();
  }); cleanup.push(() => upstream.close());
  const gateway=await startGateway({token,provider:{protocol:'chat-completions',baseUrl:upstream.baseUrl,alias:'coagent-test',upstreamModel:'fixture'}});cleanup.push(()=>gateway.close());
  const isolated = await isolatedEnvironment(gateway.baseUrl, token); cleanup.push(() => isolated.cleanup());
